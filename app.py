    ...
    try:
        if isinstance(results, list) and len(results) > 0:
            ...
            if isinstance(first, dict):
                text = first.get("text") or first.get("search_result") or first.get("raw") or ""
...
def delete_json_by_problem(problem):
    memories = load_memories()

    for i, memory in enumerate(memories):
        if memory["problem"] == problem:
            delete_memory(i)
            return True
            ...
            if not st.session_state.saved:
                if st.button("💾 Save Engineering Knowledge"):
                    ...
                    if USE_COGNEE and cognee_saved:
                        ...
                        if json_saved:
                            st.success("✅ Memory saved successfully in JSON.")
                        else:
                            st.info("Memory already exists in JSON.")

                    else:
                        if json_saved:
                            st.warning("Saved in JSON. Cognee save failed.")
                        else:
                            st.info("Memory already exists in JSON.")

            else:
                st.success("✅ This solution is already saved.")
                ...
                with col1:
                    st.success("📁 JSON Backup: Saved")

                with col2:
                    if USE_COGNEE:
                        st.success("☁️ Cognee Cloud: Enabled")
                    else:
                        st.info("☁️ Cognee Cloud: Disabled")

                st.info("🗑 JSON delete is available from the Memory History page.")
    ...
    if not memories:
        ...
        for index, memory in enumerate(memories):
            with st.expander(f"🧠 {index + 1}. {memory['problem']}"):
                ...
                if st.button("🗑 Delete JSON Memory", key=f"delete_json_{index}"):
                    delete_memory(index)
